    ...
    public partial class FrmIntegrarPacientes : Form
    {
        ...
        private void BtnBuscarPacientes_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if(IntegrarPacientes.IsBusy != true)
                {      
                    ...
                    if (res == DialogResult.Yes)
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            if (Total != 0)
                            {
                                ...
                                IntegrarPacientes.RunWorkerAsync();
        ...
        {
            if (IntegrarPacientes.IsBusy == true) //Si el proceso esta corriendo no puede voler a iniciarse 
            {
                Utils.Titulo01 = "Control de ejecución";
                Utils.Informa = "Se esta corriendo un proceso, detengalo o espere que se cierre para poder salir" + "\r";
                MessageBox.Show(Utils.Informa, Utils.Titulo01, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
        ...
        int globalCanPaciForm;
        int globalPaciExis;
        private void IntegrarPacientes_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(Conexion.conexionSQL))
                {
                    ...
                    else
                    {
                        ...
                        while (TabPacien.Read())
                        {
                                ...
                                if (TabPacienCentra.HasRows == false)
                                {
                                    ...
                                    using (SqlConnection connection3 = new SqlConnection(Conexion.conexionSQL))
                                    {
                                        ...
                                        if (TabPacienCentra1.HasRows == false)
                                        {
                                            ...
                                            "'" + TabPacien["PaciHipertenso"].ToString() + "'," +
                                            $"{Conexion.ValidarFechaNula(TabPacien["FecIniciaHta"].ToString())}" +
                                            $"{Conexion.ValidarFechaNula(TabPacien["FecIniciaDbt"].ToString())}" +
                                            $"{Conexion.ValidarFechaNula(TabPacien["FecIniciaRCV"].ToString())}" +
                                            "'" + TabPacien["PaciDiabetico"].ToString() + "'" +
                                        ...
                                        {
                                            TabPacienCentra1.Read();

                                            if (TabPacienCentra1["Nombre1"].ToString() == TabPacien["Nombre1"].ToString())
                                            {
                                                if (TabPacienCentra1["Apellido1"].ToString() == TabPacien["Apellido1"].ToString())
                                                {
                                                    if (TabPacienCentra1["Sexo"].ToString() == TabPacien["Sexo"].ToString())
                                                    {
                                                        ...
                                                        "PaciHipertenso = '" + TabPacien["PaciHipertenso"].ToString() + "', " +
                                                        $"FecIniciaHta = {Conexion.ValidarFechaNula(TabPacien["FecIniciaHta"].ToString())}" +
                                                        $"FecIniciaDbt = {Conexion.ValidarFechaNula(TabPacien["FecIniciaDbt"].ToString())}" +
                                                        $"FecIniciaRCV = {Conexion.ValidarFechaNula(TabPacien["FecIniciaRCV"].ToString())}" +
                                                        "PaciDiabetico = '" + TabPacien["PaciDiabetico"].ToString() + "' " +
                                                        ...
                                                        CodPacienPort = TabPacienCentra1["HistorPaci"].ToString();
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        Utils.Informa += "No. " + CodPacienPort + " la cual tiene el mismo documento  " + "\r";
                            ...
                            IntegrarPacientes.ReportProgress(contador);
            ...
            catch (Exception ex)
            {
                ...
                Utils.Informa += "Error: " + ex.Message + " - " + ex.StackTrace;
                MessageBox.Show(Utils.Informa, Utils.Titulo01, MessageBoxButtons.OK, MessageBoxIcon.Error);
                IntegrarPacientes.CancelAsync();
            }
        }

        private void IntegrarPacientes_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            try
            {
                if (IntegrarPacientes.CancellationPending == false)
                {
        ...
        private void IntegrarPacientes_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                TxtCanPaciFor.Text = globalCanPaciForm.ToString();
                TxtCanPaciForm.Text = globalCanPaciForm.ToString();
                TxtPaciExis.Text = globalPaciExis.ToString();
        ...
        private void BtnDetener_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (res2 == DialogResult.Yes)
                {
                    ...
                    IntegrarPacientes.WorkerSupportsCancellation = true;
                    IntegrarPacientes.CancelAsync();
