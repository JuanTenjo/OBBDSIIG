...
namespace OBBDSIIG.Forms.FrmIntegrar
{
    public partial class FrmIntegrarBiometria : Form
    {
        ...
        {
            if (IntegrarBiometria.IsBusy == true) //Si el proceso esta corriendo no puede voler a iniciarse 
            {
        ...
        private void IntegrarBiometria_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(Conexion.conexionSQL))
                {
                    ...
                    else
                    {
                        ...
                        while (TabBiometFi.Read())
                        {
                            ...
                            if (IntegrarBiometria.CancellationPending == true)
                            {
                                e.Cancel = true;
                                Utils.Titulo01 = "Control de ejecución";
                            ...
                            IntegrarBiometria.ReportProgress(contador);
            ...
            catch (Exception ex)
            {
                ...
                Utils.Informa += "Error: " + ex.Message + " - " + ex.StackTrace;
                MessageBox.Show(Utils.Informa, Utils.Titulo01, MessageBoxButtons.OK, MessageBoxIcon.Error);
                IntegrarBiometria.CancelAsync();
            }
        }

        private void IntegrarBiometria_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            try
            {
                if (IntegrarBiometria.CancellationPending == false)
                {
        ...
        private void IntegrarBiometria_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
        ...
        private void BtnDetener_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    IntegrarBiometria.WorkerSupportsCancellation = true;
                    IntegrarBiometria.CancelAsync();
                ...
                if (IntegrarBiometria.IsBusy != true) //Si el proceso esta corriendo no puede voler a iniciarse 
                {
                    ...
                    if (res == DialogResult.Yes)
                    {
                        ...
                        if (MaximoBarra >= 1)
                        {
                            ...
                            IntegrarBiometria.RunWorkerAsync();
